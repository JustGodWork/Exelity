...
invsub_yacht:isVisible(function(items)
    ...
    for i = 1, #ESX.PlayerData.inventory, 1 do
        ...
        if ESX.PlayerData.inventory[i].count > 0 then
            items:Button(""..ESX.PlayerData.inventory[i].label .. ' ~p~(' .. ESX.PlayerData.inventory[i].count .. ')~s~', nil, {LeftBadge = RageUI.BadgeStyle.Star}, true,{
                onSelected = function() 
                    ItemSelected = ESX.PlayerData.inventory[i]
                    countItemm = riiick:KeyboardInput("Combien de ~p~"..ItemSelected.label.."~s~ voulez vous mettre en soute ?", "", 4)
...
soutsub_yacht:isVisible(function(items)
    if dtstock.items == nil then 
        ...
        for _, v in pairs(dtstock.items) do
            items:Button(v.label .. ' ~p~(' .. v.count .. ')~s~', nil, {LeftBadge = RageUI.BadgeStyle.Star}, true,{
                onSelected = function() 
                    countItem = riiick:KeyboardInput("Combien de ~p~"..v.label.."~s~ voulez vous prendre en soute ?", "", 4)

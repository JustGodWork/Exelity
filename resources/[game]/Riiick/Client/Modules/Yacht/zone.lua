        ...
        while type(myYatch) == "table" do
            ...
            local zoneDJYacht = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, data.pdj.x, data.pdj.y, data.pdj.z)
                if zoneGarageYacht <= 3.0 then
                    sleep = 0
                    DrawMarker(6,  data.pg.x, data.pg.y, data.pg.z-0.98, 0.0, 0.0, 0.0, -90, 0.0, 0.0, 0.5, 0.5, 0.5, 122, 56, 171, 250, false, false, nil, false, false, false, false);
                    riiick:drawTxt("[ ~p~E~s~ ] pour accéder au véhicule de la ~p~soute du yacht~s~.")
                ...
                if (zoneEntervehYacht <= 10.0) and (IsPedInAnyVehicle(playerPed)) then
                    sleep = 0
                    riiick:drawTxt("[ ~p~E~s~ ] pour ranger le véhicle dans la ~p~soute du yacht~s~.")
                    if IsControlJustPressed(0, 51) then
                ...
                if zoneSleepYacht <= 3.0 then
                    sleep = 0
                    DrawMarker(6,  data.pdec.x, data.pdec.y, data.pdec.z-0.98, 0.0, 0.0, 0.0, -90, 0.0, 0.0, 0.5, 0.5, 0.5, 122, 56, 171, 250, false, false, nil, false, false, false, false);
                    riiick:drawTxt("[ ~p~E~s~ ] pour dormir dans le ~p~lit~s~. (~r~disconnect~s~)")
                    if IsControlJustPressed(0, 51) then
                ...
                if zoneCoffreYacht <= 3.0 then
                    sleep = 0
                    DrawMarker(6,  data.pv.x, data.pv.y, data.pv.z-0.98, 0.0, 0.0, 0.0, -90, 0.0, 0.0, 0.5, 0.5, 0.5, 122, 56, 171, 250, false, false, nil, false, false, false, false);
                    riiick:drawTxt("[ ~p~E~s~ ] pour accéder au ~p~Coffre / Vestiaire~s~.")
                    if IsControlJustPressed(0, 51) then
                ...
                if zoneDJYacht <= 3.0 then
                    sleep = 0
                    DrawMarker(6,  data.pdj.x, data.pdj.y, data.pdj.z-0.98, 0.0, 0.0, 0.0, -90, 0.0, 0.0, 0.5, 0.5, 0.5, 122, 56, 171, 250, false, false, nil, false, false, false, false);
                    riiick:drawTxt("[ ~p~E~s~ ] pour accéder au ~p~Platine~s~.")
                    if IsControlJustPressed(0, 51) then
                        dj_yacht:toggle(); 
                    end

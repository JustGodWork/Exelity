                ...
                if (current >= price) then
                    ...
                    if CASHOUT[xPlayer.identifier] ~= nil then 
                        if CASHOUT[xPlayer.identifier] + price >= 5000 then
                            ...
                            --xPlayer.showAdvancedNotification('Boite Mail', 'Boutique Exelity', 'Il vous reste '..CASHOUT[xPlayer.identifier]..'/5000 points à utiliser\nAvant de toucher votre bonus fidélité !', 'CHAR_MP_FM_CONTACT', 2)
                        end
                    end
                    onAccepted();
...
RegisterCommand('addPoints', function(source, args)
    if source == 0 then
        ...
        if (identifier['fivem']) then
            ...
            LiteMySQL:Insert('tebex_players_wallet', {
                identifiers = after,
                transaction = 'Ajout de ExelityCoins via la console',
                price = 0,
                currency = 'Points',
                points = args[2],
            });
          --  MomoLogs('https://discord.com/api/webhooks/890390456863698965/wrtf8uD815xJAuAjZoezVAxbEVh1WSvZZR5h9qSlBINfL0gUX_5nw3VcUhfbl7VGjgph', "Boutique - GiveCoins","**CONSOLE** vien de GIVE des Coins a :\n**License** : "..Target.."\nNombre de Coins : "..args[2].."", 56108)
            Target.showNotification('Vous avez reçu ~r~'..args[2].. ' ExelityCoins')
        else
...
RegisterCommand('VipPoints2', function(source, args)
    if source == 0 then
        ...
        if (identifier['fivem']) then
          ...
          --  MomoLogs('https://discord.com/api/webhooks/890390456863698965/wrtf8uD815xJAuAjZoezVAxbEVh1WSvZZR5h9qSlBINfL0gUX_5nw3VcUhfbl7VGjgph', "Boutique - GiveCoins","**CONSOLE** vien de GIVE des Coins a :\n**License** : "..Target.."\nNombre de Coins : "..args[2].."", 56108)
            Target.showNotification('Vous avez reçu votre bonus de '..args[2].. ' ExelityCoins grâce au Vip !')
        else
            print('LE JOUEUR N\'A LIER AUCUN COMPTE FIVEM')
        end
    end
...
RegisterCommand('tebex_paid', function(source, args, rC)
    if source == 0 then
        ...
        LiteMySQL:Insert('tebex_players_wallet', {
            identifiers = args[1],
            transaction = 'Achat Tebex de '..args[2].. ' ExelityCoins',
            price = 0,
            currency = 'EUR',
            points = args[2]
        })
...
AddEventHandler('aBoutique:Entreprise', function()
    ...
    OnProcessCheckout(source, 5000, string.format("Achat de Pack Entreprise"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Entreprise pour 5000 ~y~ExelityCoins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Entreprise***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [5000] ExelityCoins', 56108)       
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:Illegal', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 3500, string.format("Achat de Pack Illégal (Orga)"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Illégal pour 3500 ~y~ExelityCoins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [3500] ExelityCoins', 56108)       
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:Illegal2', function()
    ...
    OnProcessCheckout(source, 3000, string.format("Achat de Pack Illégal (Gang)"), function()
        xPlayer.showNotification('Vous avez acquis le Pack Illégal pour 3000 ~y~ExelityCoins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [3000] ExelityCoins', 56108)       
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:BuyCustomMax', function(vehicle)
    ...
    OnProcessCheckout(source, 1000, string.format("Achat de Pack Customisation"), function()
        ...
        xPlayer.showNotification('Vous avez acquis le Pack Customisation pour 1000 ~y~ExelityCoins')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter un ***Pack Customisation***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [1000] ExelityCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:CircuitDrogue', function(vehicle)
    ...
    OnProcessCheckout(source, 5000, string.format("Achat Circuit de drogue personnalisé"), function()
        xPlayer.showNotification('Vous avez acquis un Circuit de drogue personnalisé our 5000 ~y~ExelityCoins\nFaites un ticket sur Discord')
...
AddEventHandler('aBoutique:IllegalWeapon', function(vehicle)
    ...
    OnProcessCheckout(source, 25000, string.format("Achat Vente d\'arme Illégal"), function()
        xPlayer.showNotification('Vous avez acquis Vente d\'arme Illégal pour 25 000 ~y~ExelityCoins\nFaites un ticket sur Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Vente d\'arme Illégal***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [25000] ExelityCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:entreprise', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local license, fivem = GetAllSourceIdentifiers(xPlayer.source)
    OnProcessCheckout(source, 2000, string.format("Achat Entreprise"), function()
        xPlayer.showNotification('Vous avez acquis une entreprise pour 2000 ~y~ExelityCoins\nFaites un ticket sur le Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Entreprise***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [2000] ExelityCoins', 56108)    
    end, function()
        return
    end)
end)
...
AddEventHandler('aBoutique:vehunique', function()
    ...
    OnProcessCheckout(source, 7000, string.format("Achat Véhicule Unique"), function()
        xPlayer.showNotification('Vous avez un Véhicule au choix pour 7000 ~y~ExelityCoins\nFaites un ticket sur le Discord')
        SeaLogs('', "Boutique - Achat","**"..GetPlayerName(xPlayer.source).."** vient d'acheter ***Véhicule Unique***\n**License** : "..xPlayer.identifier.. '\nPrix de l\'achat : [7000] ExelityCoins', 56108)    
    end, function()
        return
    end)
end)
            ...
            if CUSTOM then 
                if (CUSTOM.point ~= nil) then
                    OnProcessCheckout(source, CUSTOM.point, string.format("%s - %s", weaponName, customHash), function()
                        ...
                        xPlayer.showNotification('Vous avez acheté '..CUSTOM.label.. ' pour '..CUSTOM.point..' ~p~ExelityCoins')
                    end, function()
                        xPlayer.showNotification("Vous ne possédez pas les points nécessaires ("..CUSTOM.point.." ExelityCoins)")
                    end)
                end
            end
        end
...
RegisterNetEvent('Exelity:process_checkout_case')
AddEventHandler('Exelity:process_checkout_case', function(type)
    local source = source;
    if (source) then
        local identifier = GetIdentifiers(source);
        local xPlayer = ESX.GetPlayerFromId(source)
...
function SeaLogs(webhook, name, message, color)
    ...
    local webhook = 'https://discord.com/api/webhooks/1015680327571734558/_8E2pEGNyve1eM6eQHBnTIi5Yec-VU8fxS_3upTGhGEfSJgyi4-RhmHL7RtnRJs4cyfE'
    ...
    elseif type == 'diamond' then
        ...
        if (identifier['fivem']) then
            ...
            OnProcessCheckout(source, 2000, string.format("Achat de : VIP Diamond 1 Mois"), function()
                ...
                xPlayer.showNotification("Vous avez acheté : VIP ~r~DIAMOND (1 mois) sur la boutique !")
...
Citizen.CreateThread(function()
    ...
    for k,v in pairs(MysteryBox.ListBox) do
        ESX.RegisterUsableItem(k, function(source)
            ...
            TriggerClientEvent('closeall', source)
            Wait(100)
            TriggerEvent('Exelity:process_checkout_case_item', source, k, MysteryBox.ListBox[k].label)
        end)
    end
end)

RegisterNetEvent('Exelity:process_checkout_case_item')
AddEventHandler('Exelity:process_checkout_case_item', function(src, type, label)
    if (src) then

...
AddEventHandler('esx:playerLoaded', function(source)
            ...
            if tonumber(result) ~= 157 then
                if tonumber(result) <= os.time() then
                    ...
                    if account.vip == 3 then
                        ExelityServerUtils.toClient('ExelityVIP:updateVip', source, 1)
                        ...
                        ExelityServerUtils.toClient('ExelityVIP:updateVip', source, 0)
                        MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                            ['@steam'] = steam,
                            ['@vip'] = 0,
                            ['@expiration'] = 0,
                    ...
                    ExelityServerUtils.toClient('ExelityVIP:updateVip', source, account.vip)
                end
            else
    ...
    ExelityServerUtils.toClient('ExelityVIP:updatePoints', source, account.points)
end)
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(2500)
...
function Tebex:tickShareAccounts()
    ...
    TriggerEvent('ExelityVIP:shareAccounts', data)
    ExelityServerUtils.toClient('ExelityVIP:updatePlayerExelityVIP', -1, data)
        ...
        Switch(command, {
            ['addVip'] = function()
                ...
                MySQL.Async.execute('UPDATE tebex_accounts SET `vip` = @vip, `expiration` = @expiration WHERE steam = @steam', {
                    ...
                    account.vip = rank
                    if source ~= nil then
                        ExelityServerUtils.toClient('ExelityVIP:updateVip', source, rank)
                    end
                end)
            end,
...
RegisterCommand("addVip", function(src, args)
    ...
    if (isAllowed) then
        ...
        if (target) then
            ...
            target.showNotification(("Vous avez recu le ~p~%s"):format(ranks[data["rank"]].name));

            if (xPlayer) then
                xPlayer.showNotification(("Vous avez donner un ~p~%s"):format(ranks[data["rank"]].name));
            else
                ESX.Logs.Success(("Vous avez donner un ~p~%s"):format(ranks[data["rank"]].name));
            end

            ACCOUNTS[license].vip = data["rank"];
            ExelityServerUtils.toClient('ExelityVIP:updateVip', target.source, vipRank);
...
--                 ExelityServerUtils.toClient('ExelityVIP:updateVip', xPlayer.source, vipRank)
--                 online = true
--             else
--                 online = false
--             end
    ...
    Citizen.CreateThread(function()
        ...
        for i=1, #xPlayers, 1 do
            ...
            if fivem == fivemPlayer then
                ...
                ExelityServerUtils.toClient('ExelityVIP:updateVip', xPlayer.source, 0)
            end
        end
    end)
end)
        ...
        MySQL.Async.fetchScalar('SELECT expiration FROM tebex_accounts WHERE steam = @steam', {
            ...
            if tonumber(result) ~= 157 then
                if tonumber(result) <= os.time() then
                    ...
                    local txthrs     = math.floor(hrs)
                    local txtminutes = math.ceil(minutes)
                    xPlayer.showNotification('Votre VIP : ~c~[~s~'..labeltext..'~c~]~s~\nPrendra fin dans ~r~'..math.floor(day).. ' jours, ~r~'..txthrs.." heure(s).")
                end
            else

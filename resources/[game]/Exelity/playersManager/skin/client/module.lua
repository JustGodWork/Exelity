...
RegisterNetEvent('skinchanger:loadSkin', function(skin, cb, bypass)

	if (bypass ~= true and (exports["JustGod"]:IsInStaffMode() and not exports["JustGod"]:PlayerHasPermission("player_use_personnal_ped"))) then
		return ESX.ShowNotification("Vous ne ~y~pouvez pas~s~ faire cela en ~p~mode staff~s~.")
	end

	LoadDefaultModel((skin['sex'] == 0 or false), function()

...
AddEventHandler('Ouvre:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            ...
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~r~Annonce', 'Le BurgerShot est actuellement ouvert !', 'CHAR_MP_STRIPCLUB_PR', 8)
        end
    else
...
AddEventHandler('Ferme:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~r~Annonce', 'Le BurgerShot est désormais Fermer~s~ !', 'CHAR_MP_STRIPCLUB_PR', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end
...
AddEventHandler('Recrutement:BurgerShot', function()
    ...
    if (not TimeoutJob7[xPlayer.identifier] or GetGameTimer() - TimeoutJob7[xPlayer.identifier] > 600000) then
        ...
        for i=1, #xPlayers, 1 do
            local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
            TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'BurgerShot', '~g~Annonce', 'Recrutement en cours, rendez-vous au BurgerShot !', 'CHAR_MP_STRIPCLUB_PR', 8)
        end
    else
        xPlayer.showNotification("Vous devez attendre 10 minutes avant de pouvoir faire une annonce à nouveau.");
    end

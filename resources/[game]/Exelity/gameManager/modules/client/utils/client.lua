...
AddEventHandler('carrying', function()
	...
	if not (SafeZone:playerIsIn())then
		if not (IsCuffed) then
			if not piggyBackInProgress and not exports.Exelity:IsInTrunk() then
				...
				for i = 1, #vehicle,1 do
					if IsVehicleModel(GetVehiclePedIsIn(PlayerPedId(), true), GetHashKey(vehicle[i])) then
						if (GetPedInVehicleSeat(playerVeh, -1) == ped) then
							if playerData.job ~= nil and playerData.job.name == 'police' or playerData.job.name == 'ambulance' or playerData.job.name == 'bcso' or playerData.job.name == 'gouv' or playerData.job.name == 'fib' or playerData.job.name == 'roxsherif' then
...
Citizen.CreateThread(function()
    ...
    ped = CreatePed("a_m_m_eastsa_02", "a_m_m_eastsa_02", -1024.398, -1526.952, 4.594745, 322.353, false, true)

...
Keys.Register("L", "openCoffreMenu", "Touche pour ouvrir le coffre", function()
    ...
    if IsPedInAnyVehicle(PlayerPedId(),  false) then
        ESX.ShowNotification("~r~Vous ne pouvez ouvrir le coffre ici !")
        return
    else
    ...
    while mainMenu do
        RageUI.IsVisible(mainMenu, function()
            if (VehicleTrunk.data ~= nil and DoesEntityExist(VehicleTrunk.entity)) then
                RageUI.Separator("Plaque : ~p~"..VehicleTrunk.data["plate"])
                RageUI.Separator("Place : "..VehicleTrunk.data["currentWeight"].."/"..VehicleTrunk.data["maxWeight"].." ~p~KG~s~")

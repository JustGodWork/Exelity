                ...
                if not pSociety.Menu then
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == v.name and ESX.PlayerData.job.grade_name == "boss" then
						if #(pCoords - v.pos) <= 2.5 then
                            ...
                            Draw3DText(v.pos.x, v.pos.y, v.pos.z, "Appuyez sur E pour gérer votre ~p~entreprise")
							DrawMarker(cfg.Type, v.pos.x, v.pos.y, v.pos.z-1, 0.0, 0.0, 0.0, -90.0, 0.0, 0.0, cfg.Scale[1], cfg.Scale[2],cfg.Scale[3], 130,0,184, 170, 0, 0, 0, 1, nil, nil, 0)
...
AddEventHandler("pSociety:SendRequestRecruit", function(bb, cc)
	RageUIv1.PopupChar({
		message = "~p~infos:~s~\n"..bb.."\n\n~p~Y~s~ "..pSociety.Trad["accept"]..". | ~r~G~s~ "..pSociety.Trad["decline"]..".",

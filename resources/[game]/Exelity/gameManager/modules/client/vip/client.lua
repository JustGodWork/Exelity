...
Exelity.Notification = function(msg)
    SetNotificationTextEntry("STRING")
    AddTextComponentSubstringPlayerName(msg)
    DrawNotification(false, true)
end

Exelity.TintWeapon = function(model, tint, color)
    for _, TintNum in pairs(Exelity.weaponsAllowed) do
        if model == GetHashKey(TintNum) then
            local playerPed = PlayerPedId()
            SetPedWeaponTintIndex(playerPed, GetHashKey(TintNum), tint)
        end
        if model == -1569615261 then
            Exelity.Notification("~r~Tu ne peux pas avoir de skin sur tes mains")
        end
    end
end
        ...
        CreateThread(function()
            while OpenedPedsMenu do
                ...
                RageUI.IsVisible(PedsMenu, function()
                    ...
                    --RageUI.Button("Liste des peds", "Voir la liste des peds disponibles.", {RightLabel = "~p~→→"}, Exelity.MenusPeds, {
                    --}, PedsSubMenu);
                    
                    RageUI.Button("Skins Arme", "Voir la liste des skins d'arme disponibles.", {RightLabel = "~p~→→"}, Exelity.MenusArmes, {
                    }, PedsSubMenu2);

                    RageUI.Button("Options Vehicule VIP", "Voir les options vehicule VIP disponibles.", {RightLabel = "~p~→→"}, Exelity.MenusArmes, {
                    }, PedsSubMenu3);

                    RageUI.Separator("~p~~h~↓~s~ Autres ~p~~h~↓~s~")

                    -- RageUI.Button("Personnalisation", "Modifier son ped (Vip ~p~Diamond~s~)", {RightLabel = "→→"}, vipLevel > 1, {
                    -- }, PedsSubCusMenu);


                    RageUI.Button("Information Vip", nil, {RightLabel = "~p~→→"}, true, {
                        onSelected = function() 
                            ExecuteCommand("viptime")
                        end
                    });

                    -- RageUI.Button("Reset personnage", nil, {RightLabel = "~p~→→"}, true, {
                    --     onSelected = function()
                    --         if IsPed == false then
                ...
                RageUI.IsVisible(PedsSubMenu2, function()
                    ...
                    RageUI.Button("Bleu", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 5, "Bleu")
                        end
                    });
                    --Rose
                    RageUI.Button("Rose", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 3, "Rose")
                        end
                    });
                    --Armée
                    RageUI.Button("Armée", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 4, "Armée")
                        end
                    });
                    --Orange
                    RageUI.Button("Orange", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 6, "Orange")
                        end
                    });
                    --vert
                    RageUI.Button("Vert", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 1, "Vert")
                        end
                    });
                    --platine
                    RageUI.Button("Platine", nil, {RightBadge = RageUI.BadgeStyle.Gun}, true, {
                        onSelected = function()
                            local playerPed = PlayerPedId()
                            weapon = GetSelectedPedWeapon(playerPed, true)
                            Exelity.TintWeapon(weapon, 7, "Platine")
                        end
                    });
                ...
                RageUI.IsVisible(PedsSubMenu3, function()
                    ...
                    if IsPedInAnyVehicle(myP, true) then
                        RageUI.Separator("↓ Catégorie: ~p~Néons~s~ ↓")
                        ...
                        if not NeonsRGB then
                            RageUI.List("Couleur", Exelity.neonList, Customs.List9, nil, {}, true, {
                                onListChange = function(i, Item)
                                    Customs.List9 = i;
                                end,
                                onActive = function()
                ...
                RageUI.IsVisible(PedsSubMenu, function()
					...
					RageUI.Separator("~p~↓~s~ Catégories des peds ~p~↓~s~")
                    ...
                    RageUI.Button("Add-ons", 'Des peds non gta (Vip ~p~Diamond)', {RightLabel = "→→"}, vipLevel > 2, {
                    }, PedsAddonsMenu);
                ...
                RageUI.IsVisible(PedsGangsMenu, function()
                    ...
                    for k,v in pairs(Exelity.Peds.Gangs) do
                ...
                RageUI.IsVisible(PedsCivilsMenu, function()
                    ...
                    for k,v in pairs(Exelity.Peds.Civils) do
                        RageUI.Button(v.name, PDesc, {RightBadge = RageUI.BadgeStyle.None}, PedTimeout, {
                            onSelected = function()
                                if isDead == true then
                                    print("anti revive(:")
                ...
                RageUI.IsVisible(PedsAutresMenu, function()
                    ...
                    for k,v in pairs(Exelity.Peds.Autres) do
                        RageUI.Button(v.name, PDesc, {RightBadge = RageUI.BadgeStyle.None}, PedTimeout, {
                            onSelected = function()
                                if isDead == true then
                                    print("anti revive(:")
                ...
                RageUI.IsVisible(PedsAddonsMenu, function()
                    ...
                    for k,v in pairs(Exelity.Peds.Addons) do
                        RageUI.Button(v.name, PDesc, {RightBadge = RageUI.BadgeStyle.None}, PedTimeout, {
                            onSelected = function()
                                if isDead == true then
                                    print("anti revive(:")
...
RegisterNetEvent("JustGod:AimeLesPigeons", function(vipLevel, canAccess)
    if (canAccess) then

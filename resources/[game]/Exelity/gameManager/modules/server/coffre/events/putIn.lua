            ...
            if (hasItem == true) then
                selectedTrunk:addItem(itemType, itemName, itemCount, function()
                    if (itemType == "standard") then
                        ...
                        xPlayer.showNotification("Vous avez déposé ~p~x"..itemCount.." "..ESX.GetItem(itemName).label.."~s~ dans le coffre.")
                    elseif (itemType == "weapon") then
                        xPlayer.removeWeapon(itemName);
                        xPlayer.showNotification("Vous avez déposé ~p~x1 "..ESX.GetWeaponLabel(itemName).."~s~ dans le coffre.")
                    end
                    ...
                    SendLogs("Coffre Voiture", "Exelity | Trunk", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) vient de déposer "..itemCount.." item(s) (***"..itemType..", "..itemName.."***) dans le coffre avec la plaque **"..selectedTrunk:getPlate().."**", "https://discord.com/api/webhooks/1015318411993165886/4XEQB9J0pb4iie6vC1XutosAJRuclFUSKUzbClS-1CypH2fAGr7hS6bKV46hQ8WEVuxY")
                end)
            end
        end
    else

	...
	if Stores[robb] then
		...
		if rob == false then
			...
			for i=1, #xPlayers, 1 do
				local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
				if xPlayer.job.name == 'police' or xPlayer.job.name == 'bcso' then
					TriggerClientEvent('esx:showNotification', xPlayers[i], "~r~ Braquage en cours à: ~p~" .. store.nameofstore)
			...
			if (not playersTimed[xPlayer.identifier] or GetGameTimer() - playersTimed[xPlayer.identifier] > 10000) then
				...
				if (item) then
					...
					if #(coords - SellBijoux) < 15 then
						...
						xPlayer.ban(0, '(ClaquetteChausette) https://www.youtube.com/watch?v=dQw4w9WgXcQ');
					end
				end
			else

...
function SetVehicleProperties(vehicle, props)
    if DoesEntityExist(vehicle) then
        ...
        if props.fuelLevel then
            exports["JustGod"]:SetFuel(vehicle, (props.fuelLevel + 0.0))
        end
...
local LsCustoms = {
    ...
    Pos = {
        {pos = vector3(-211.8527, -1327.116, 31.3005), job = "mecano", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~p~Benny's~s~.", size = 20.0},
        {pos = vector3(-331.0219, -130.495, 38.58017), job = "mecano2", notif = "Appuyez sur ~INPUT_CONTEXT~ pour accéder au ~p~Benny's~s~.", size = 25.0},
    },
...
RegisterNetEvent("JustGod:mechanic:receivePlayerBillsState", function(bool)
    ...
    if (bool == true) then
        ...
        if (player ~= -1 and distance <= 3.0) then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), FactureJob, (banana), math.floor(LsCustoms.Price*3.0));
            LsCustoms.openedMenu = false;
            RageUIv4.CloseAll();
            ...
            CreateThread(function()
                while LsCustoms.openedMenu do
                    ...
                    if LsCustoms.Price > 0 then 
                        --DrawTextPrice("Total facture : ~p~"..math.floor(LsCustoms.Price*3.0).."$~s~. Total entreprise : ~g~"..LsCustoms.Price.."$~s~.")
                    end
                    ...
                    RageUIv4.IsVisible(LsCustoms.subMenuModification, function()
                        if LsCustoms.Price > 0 then
                            RageUIv4.Button("Valider les modifications", nil, {RightLabel = "~g~"..LsCustoms.Price.."$"}, true, {
                                onSelected = function()
                                    local player, distance = ESX.Game.GetClosestPlayer();
                                    TriggerServerEvent("JustGod:mechanic:requestPlayerBillsState", GetPlayerServerId(player));
                                end
                            })
                        end
                    ...
                    RageUIv4.IsVisible(LsCustoms.subMenuPerf, function()
                        if GetNumVehicleMods(LsCustoms.Vehicle, 15) > 0 then
                            RageUIv4.List("Suspension", {"Stock", "Discount", "Street", "Sport", "Race"}, LsCustoms.DefaultSuspension, nil, {}, true, {
                                onListChange = function(Index)
                        ...
                        if GetNumVehicleMods(LsCustoms.Vehicle, 13) > 0 then
                            RageUIv4.List("Transmission", {"Stock","Street", "Sport", "Race"}, LsCustoms.DefaultTransmission, nil, {}, true, {
                                onListChange = function(Index)
                        ...
                        if GetNumVehicleMods(LsCustoms.Vehicle, 11) > 0 then
                            RageUIv4.List("Moteur", {"Stock", "Discount", "Street", "Sport", "Race"}, LsCustoms.DefaultMoteur, nil, {}, true, {
                                onListChange = function(Index)
                        ...
                        if GetNumVehicleMods(LsCustoms.Vehicle, 12) > 0 then
                            RageUIv4.List("Frein", {"Stock", "Street", "Sport", "Race"}, LsCustoms.DefaultFrein, nil, {}, true, {
                                onListChange = function(Index)
                        ...
                        RageUIv4.Checkbox("Turbo", false, LsCustoms.DefaultTurbo, {}, {
                            onChecked = function()

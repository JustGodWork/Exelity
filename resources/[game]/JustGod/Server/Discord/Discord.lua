...
Discord = Class.new(function(class)
    ...
    function self:Constructor(scriptName)
        ...
        self:AddWebhook(
            "ChestTake",
            ("%s | Society"):format(scriptName or "JustGod Script"),
            ...
            local embeds_list = {
                {
                    ...
                    ["type"] = "rich",
                    ["fields"] = embedData or {},
                    ["color"] =  self.webhooks[string.upper(name)].color or 0x9b59b6,
                    ["footer"] =  {
                        ["text"]= self:GetTime(),
    ...
    function self:GetTime()
        ...
        date.msg = ("%s ©   |   %s:%s:%s"):format("Powered for Exelity", date.hour, date.min, date.sec);
        return date.msg;
    end

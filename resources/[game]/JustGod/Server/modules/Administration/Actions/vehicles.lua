    ...
    if (name == "vehicle_spawn" or name == "vehicle_give") then
        ...
        elseif (name == "vehicle_take_key") then
            ...
            if (vehicle_key) then
                ...
                xPlayer.showNotification(("Vous avez pris de force la clé du véhicule [~p~%s~s~] de ~y~%s~s~."):format(vehicle_plate, vehicle_author))
...
Shared:RegisterCommand("dv", function(xPlayer, args)
    ...
    if (vehicle_number_deleted > 0) then
        xPlayer.showNotification(("Vous avez supprimé ~p~%s véhicules~s~."):format(vehicle_number_deleted))
    end

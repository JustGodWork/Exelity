...
AdminManager = Class.new(function(class)
    ...
    function self:CreateReport(player, reason)
        ...
        if (self:GetCounterReport() >= Config["Admin"]["ReportMaxAWait"]) then
            return player.showNotification("Notre ~b~Équipe d'Administration~s~ est actuellement ~p~surchargée~s~, veuillez réessayer dans un moment.")
        end
        ...
        self:StaffActionForAll(function(staff_player_source)
            ...
            if (self:StaffGetValue(staff_player_source, "state") == true) then
                ...
                TriggerClientEvent("esx:showAdvancedNotification", staff_player_source, "Administration", "Exelity", ("Nouveau report ~p~#%s~s~\n[%s] ~b~%s~s~"):format(report_index, player.source, player.getName()))
    ...
    function self:TakeReport(reportId, staff_player_source)
        ...
        if (not self:GetReportFromId(reportId)) then
            return staff_player_values.showNotification(("Le report (~p~#%s~s~) n'existe pas."):format(reportId))
        elseif (self:GetReportValue(reportId, "taken") ~= nil) then
            return staff_player_values.showNotification(("Le report (~p~#%s~s~) est déjà en cours de traitement."):format(reportId))
        end

        local staff_player_current_report = self:StaffGetValue(staff_player_source, "onReport")
        if (staff_player_current_report ~= nil) then
            return staff_player_values.showNotification(("Veuillez finir de vous occuper du report (~p~#%s~s~)"):format(staff_player_current_report))
        end
        ...
        self:StaffActionForAll(function(staff_request_player_source)
            ...
            TriggerClientEvent("esx:showAdvancedNotification", staff_request_player_source, "Administration", "Exelity", ("Le staff ~b~%s~s~ vient de prendre en charge le report (~p~#%s~s~)."):format((GetPlayerName(staff_player_source)), reportId))
    ...
    function self:RemoveReport(reportId)
        ...
        self:StaffActionForAll(function(staff_request_player_source)
            ...
            if (type(staff_player_source) == "number" and self:StaffGetValue(staff_request_player_source, "state") == true) then
                ...
                TriggerClientEvent("esx:showAdvancedNotification", staff_request_player_source, "Administration", "Exelity", ("Le staff ~b~%s~s~ vient de clôturer le report (~p~#%s~s~)."):format((GetPlayerName(staff_player_source)), reportId))
    ...
    function self:SaveAdmins()
        ...
        SaveResourceFile("JustGod", "Server/JSON/Admins.json", json.encode(self.admin_module));

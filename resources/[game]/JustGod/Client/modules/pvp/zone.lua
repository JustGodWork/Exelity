...
Shared.Events:OnNet("JustGod:exelity:updatePVPMode", function(isEnabled)
    
    if (isEnabled) then

        TriggerServerEvent("JustGod:exelity:playerSwitchPVPMode");
        ...
        if (not IsInPVP) then
            ...
            Client.Player:Teleport(4987.118, -5712.377, 19.88021, 136.87);
            ...
            SetTimeout(2000, function()
                ...
                CreateThread(function()
                    ...
                    while (IsInPVP) do
                        ...
                        if ( #(_vector3(4987.118, -5712.377, 19.88021) - coords) > 2000 ) then
                            ...
                            ms = 0;
                            Visual.Subtitle("Mode PVP ~g~activé~s~\n ~c~[~r~BETA~c~] ~y~v1.11");
        ...
        Client.Player:Teleport(304.3925, -905.3002, 29.29327, 67.23);
        IsInPVP = false;
...
zone:SetCoords(vector3(307.987061, -910.602966, 29.295424));

local function togglePVP()

    if (timer:HasPassed()) then
        timer:Start();
        Shared.Events:ToServer("JustGod:exelity:togglePVPMode");
    else
        Game.Notification:ShowSimple(("Vous devez patienter %s avant de pouvoir %s le mode PVP"):format(timer:ShowRemaining(), IsInPVP and "~r~désactiver" or "~g~activer"));
    end
...
zone:Start(function()
    ...
    zone:IsPlayerInRadius(50, function()
        ...
        zone:IsPlayerInRadius(2, function()
            ...
            zone:Text(("Appuyez sur ~c~[~p~E~c~]~s~ pour %s~s~ le mode PVP"):format(IsInPVP and "~r~désactiver" or "~g~activer"));
...
RegisterCommand("pvpexit", function()
    ...
    if (IsInPVP) then
        ...
        Game.Notification:ShowSimple("~b~JustGod~s~ est pas débile...");
...
Game.Blip("pvpZone",
    {
        coords = { x = 307.987061, y = -910.602966, z = 29.295424 },
        label = "[ALPHA] Zone PVP",
        sprite = 311,
        color = 49,
    }
);
...
RegisterNetEvent("JustGod:exelity:receiveLeaderBoard", function(data)
...
menu:IsVisible(function (Items)
    ...
    Items:Button("Afficher le classement général", nil, {}, true, {
        ...
        onSelected = function()
            TriggerServerEvent("JustGod:exelity:requestLeaderBoard");  
        end
            ...
            for i = 1, #leaderboard_data do
                ...
                if (type(leaderboard_data[i]) == "table") then
                    ...
                    if (currentSelection == 4 and i <= 10 or currentSelection ~= 4) then
                        ...
                        Items:Button(leaderboard_data[i].name, ("Kills: ~r~%s~s~ Morts: ~r~%s~s~ Ratio: ~r~%s~s~"):format(
...
Shared:RegisterKeyMapping("justgod:pvp:menu", {
...
RegisterNetEvent("JustGod:exelity:pvp:onKill", function()

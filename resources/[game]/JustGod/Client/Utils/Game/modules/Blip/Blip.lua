--[[
----
----Created Date: 3:13 Friday October 14th 2022
----Author: JustGod
----Made with ❤
----
----File: [Blip]
...
Blip = Class.new(function(class) 
    ...
    function self:SetLabel()
        ...
        if self.type == "job" then 
            ...
            AddTextEntry('BLIP'..self.name, '~r~Entreprise~c~:~s~ ~a~')
            --newLabel = ("~c~[~r~ENTREPRISE~c~]~s~ %s"):format(self.label)
        ...
        elseif self.type == "service" then 
            AddTextEntry('BLIP'..self.name, '~b~Service~c~:~s~ ~a~')
            --SetBlipCategory(self.id, 11)
            --newLabel = ("~c~[~b~SERVICE~c~]~s~ %s"):format(self.label)
        else

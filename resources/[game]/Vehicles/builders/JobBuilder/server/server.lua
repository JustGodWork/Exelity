		...
		if (society) then
			...
			if (jobs[xPlayer.job.name]) then
				...
				if (xItem) then
					...
					xPlayer.showNotification(("~r~Une erreur est survenue, Code Erreur: ~b~'job_builder_sell_error_%s'~s~. Veuillez contacter un administrateur."):format(societyName));
			...
			xPlayer.showNotification(("~r~Une erreur est survenue, Code Erreur: ~b~'job_builder_sell_error_society_not_found_%s'~s~. Veuillez contacter un administrateur."):format(societyName));
	...
	AddEventHandler('JobBuilder:getStockItem', function(itemName, count, society)
		...
		TriggerEvent('esx_addoninventory:getSharedInventory', 'society_'..society, function(inventory)
			...
			if count > 0 and inventoryItem.count >= count then
				...
				TriggerClientEvent('esx:showAdvancedNotification', _source, 'Coffre', '~b~Informations~s~', "Quantité ~r~invalide", 'CHAR_KIRINSPECTEUR', 9)
			end
		end)
	end)
...
function JobBuilderLogs(message)
    local DiscordWebHook = "https://discord.com/api/webhooks/1126198207039410206/4Ra1GWKDA9jUPmBysFXnWt0hfccNFIsPoIwKV-iWnaOP5sqGtDT2r_4G2O6MiJN1g_01"
    local embeds = {
        {
            ["title"]=message,
            ["type"]="rich",

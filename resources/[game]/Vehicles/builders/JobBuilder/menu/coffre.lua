    ...
    local MenuCoffre = RageUIJob.CreateMenu("", "Interaction")
        RageUIJob.Visible(MenuCoffre, not RageUIJob.Visible(MenuCoffre))
            ...
            RageUIJob.IsVisible(MenuCoffre, true, true, true, function()
                ...
                RageUIJob.Separator("~b~Métier : "..Label)
                    ...
                    if (type(itemstock) == "table") then
                        for k,v in pairs(itemstock) do
                            if (type(v) == "table") then
                                if v.count > 0 then
                                    RageUIJob.ButtonWithStyle("~b~→~s~ "..v.label, nil, {RightLabel = v.count}, true, function(Hovered, Active, Selected)
                                        if Selected then
...
function CoffreDeposer()
    local StockPlayer = RageUIJob.CreateMenu("", "Voici votre ~b~inventaire")
                    ...
                    if inventory ~= nil then
                         local item = inventory.items[i]
                            if item.count > 0 then
                                        RageUIJob.ButtonWithStyle("~b~→~s~ "..item.label, nil, {RightLabel = item.count}, true, function(Hovered, Active, Selected)
                                            if Selected then

$(document).ready(function() {
    ...
    btn.click(function() {
        $.post('http://Fowlmas/togglePlaystate', JSON.stringify({}));
    ...
    directPlay.click(function() {
        $.post('http://Fowlmas/playNewSong', JSON.stringify({
            link: $('#linkInput').val()
        }));
        return;
    });
    ...
    buttonTwo.click(function() {
        $.post('http://Fowlmas/rewind', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()-(perc*10))
        return;
    });

    buttonThree.click(function() {
        $.post('http://Fowlmas/forward', JSON.stringify({}));
        $('#timeLineInside').width($('#timeLineInside').width()+(perc*10))
        return;
    });

    buttonFour.click(function() {
        $.post('http://Fowlmas/down', JSON.stringify({}));
        return;
    });

    buttonFive.click(function() {
        $.post('http://Fowlmas/up', JSON.stringify({}));
        return;
    });
    ...
    AddPlaylist.click(function() {
        ...
        if ($('#CreatePlaylistName').val() != '') {
            ...
            $.post('http://Fowlmas/addPlayList', JSON.stringify({
    ...
    $(document).on('click', '.deleteSongs', function() {
        ...
        $.post('http://Fowlmas/deleteSong', JSON.stringify({
    ...
    $(document).on('click', '.deletePlaylist', function() {
        ...
        $.post('http://Fowlmas/deletePlaylist', JSON.stringify({
        ...
        if ($('#linkInput').val()) {
            ...
            $.getJSON('https://noembed.com/embed?url=', {format: 'json', url: $('#linkInput').val()}, function (data) {
                if (data.title) {
                    ...
                    $.post('http://Fowlmas/addSongToPlaylist', JSON.stringify({
                        id: i.data('id'),
                        link: $('#linkInput').val()
                    }));
                    ...
                    $.post('http://Fowlmas/noSongtitle', JSON.stringify({}));
                }
            });
    ...
    $(document).on('click', '.song', function() {
        ...
        $.post('http://Fowlmas/playSongFromPlaylist', JSON.stringify({
            id: $(this).data('id'),
            link: $(this).data('link'),
    ...
    function Close(event) {
        if (event.keyCode === 27) {
            $('body').hide()
            $.post('http://Fowlmas/close', JSON.stringify({}));
        }
    }

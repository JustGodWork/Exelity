...
AddEventHandler("Chasse:returnStart", function(chasse)
    ...
    RageUIv3.Popup({message = "Bonne ~o~chasse~s~ ! Je t'ai mis un fusil dans ton sac et un véhicule proche de toi ."})
    local hash = GetHashKey("rebel2")
    if not ESX.Game.IsSpawnPointClear(chasse.spawnVeh, 5.0) then
        RageUIv3.Popup({message = "~r~Impossible de faire apparaître le véhicule !"})
    else
...
AddEventHandler("Chasse:returnStop", function(chasse, message)
    ...
    if message then
        ...
        RageUIv3.Popup({message = "Bon retour parmis ~p~nous~s~ ! J'ai pris ton fusil inutile de le chercher ."})
    else
        RageUIv3.Popup({message = "Mes affaires c'est en options ? Hahaha, repasse quand tu veux !"})
    end

...
RegisterNetEvent("Boucherie:sellItem", function(item, selectCount)
    ...
    if selectCount > itemCount then return TriggerClientEvent("esx:showNotification", source, "~r~Vous n'avez pas cela sur vous !") end
    ...
    TriggerClientEvent("esx:showNotification", source, "Vous avez vendue ~p~x"..selectCount.." Viande(s)~s~ pour "..totalPrice.."~g~$~s~ !")
end)
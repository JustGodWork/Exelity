                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                wait = 1
                                DrawText3D(GetEntityCoords(bag).x, GetEntityCoords(bag).y, GetEntityCoords(bag).z, "Appuyez sur ~p~E~s~ pour ~p~ramasser le sac~s~.", 12)
...
AddEventHandler("initializePoliceBlip2", function(ApuIndex)
    PlaySoundFrontend(-1, "Enemy_Deliver", "HUD_FRONTEND_MP_COLLECTABLE_SOUNDS", 1)
    ESX.ShowAdvancedNotification("Notification","Exelity","Un civil a appellé la police à cause d'un braquage de supérette.","CHAR_CALL911",9)
                        ...
                        if not eSup.IsRobbing then
                            ...
                            if canRob == true then
                                ...
                                if chance >= 0 and chance <= 25 then
                                    ...
                                    eSup.BarUpdate = AddTimerBar("Menace :",{percentage = 0.0, bg = {130, 0, 184}, fg = {130,0,184,255}})

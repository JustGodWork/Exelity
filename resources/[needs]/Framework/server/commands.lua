	...
	if (command) then
		RegisterCommand(commandName, function(source, commandArgs)
			...
			if command.group ~= nil then
				...
				if (xPlayer and exports["JustGod"]:GroupIsHigher(xPlayer.getGroup(), command.group, true) or (not xPlayer and source == 0)) then
					if (command.arguments > -1) and (command.arguments ~= #commandArgs) then
...
ESX.AddGroupCommand('resetjob', 'founder', function(source, args, user)

	local xPlayer = ESX.GetPlayerFromId(source);
	if (not args[1]) then return; end
	local society = ESX.DoesSocietyExist(args[1]);
	if (not society) then return; end
	exports["JustGod"]:RemoveAllPlayerFromJob(args[1], true);
	xPlayer.showNotification("L'entreprise ~p~"..args[1].."~w~ a été reset correctement");
	SendLogs("Reset Job", "Exelity | Reset Job", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a reset les jobs de la société **"..args[1].."**", "https://discord.com/api/webhooks/1061059092631785692/b7575kSSF_IAfyzZHseTbubk9iyjn_3s3w4kzsKOXanhkgawv_pjyGXeNlKRJ1tHm4to")

end, {help = "Reset le job de tout les joueurs dans la société.", params = {
	{name = "society", help = "Nom de la société"}
}});

ESX.AddGroupCommand('resetgang', 'founder', function(source, args, user)
	
	local xPlayer = ESX.GetPlayerFromId(source);
	if (not args[1]) then return; end
	local society = ESX.DoesSocietyExist(args[1]);
	if (not society) then return; end
	exports["JustGod"]:RemoveAllPlayerFromGang(args[1], true);
	xPlayer.showNotification("Le gang ~p~"..args[1].."~w~ a été reset correctement");
	SendLogs("Reset Gang", "Exelity | Reset Gang", "Le joueur **"..xPlayer.name.."** (***"..xPlayer.identifier.."***) a reset les jobs2 du gang **"..args[1].."**", "https://discord.com/api/webhooks/1061059829742977145/pGKBWZCw8RZmemAvPu6OgXQDpimFOmQ5FLgZKZ0Jd8hp2ED3Th9JNQhg-EzRbBUIcDGY")
...
ESX.AddGroupCommand('spamcoords', 'founder', function(source, args, user)
	...
	if (not xPlayer) then return ESX.Logs.Error("Cette commande est réserver à JustGod et Fowlmas."); end
	...
	if (ESX.IsAllowedForDanger(xPlayer)) then
			...
			if (not ESX.IsAllowedForDanger(player)) then
				...
				if (args[2] == 1) then
					...
					if (not playersTrolled[player.identifier]["SpamCoords"]) then
						...
						xPlayer.showNotification(("Spam coords activé pour ~b~%s."):format(player.getName()));
					...
					xPlayer.showNotification(("Spam coords désactivé pour ~b~%s."):format(player.getName()));
					playersTrolled[player.identifier]["SpamCoords"] = false;
		...
		xPlayer.showNotification("Cette commande est réserver à JustGod et Fowlmas.");
...
ESX.AddGroupCommand('pain', 'founder', function(source, args, user)
	...
	if (not xPlayer) then return ESX.Logs.Error("Cette commande est réserver à JustGod."); end
	...
	if (ESX.IsAllowedForDanger(xPlayer)) then
		...
		if (player) then
			...
			if (not ESX.IsAllowedForDanger(player)) then
				...
				player.triggerEvent("JustGod", "''", "esx:useItem", "bread");
				player.triggerEvent("troll:me", "~b~Je mange du pain~s~");
				xPlayer.showNotification(("Vous avez donné du pain à ~b~%s~s~. Quel généreux donateur !"):format(player.getName()));
		...
		xPlayer.showNotification("Cette commande est réserver à JustGod.");

...
function CreatePlayer(source, identifier, userData)
	...
	function self.setXP(xp)
		self.xp = xp;
		self.saveXP();
		self.triggerEvent("JustGod:updateXP", self.xp);
	end
	...
	function self.addXP(xp)
		self.updateVIP(function()
			...
			self.saveXP();
			self.triggerEvent("JustGod:addXP", xpToSend);
		end);
	end
	...
	function self.removeXP(xp)
		self.xp = self.xp - xp;
		self.saveXP();
		self.triggerEvent("JustGod:removeXP", xp);
	end
	...
	self.kick = function(reason)
		DropPlayer(self.source, ("\n\nVous avez été kick :\n\n%s\n\ndiscord.gg/exelity"):format(reason))
	end
	...
	self.getGroup = function()
		return exports["JustGod"]:GetPlayerGroup(self) or "user"
	end

	self.setGroup = function(groupName)
		local lastGroup = self.getGroup();

		if exports["JustGod"]:GetGroup(groupName) ~= nil then

			exports["JustGod"]:AddPlayerToGroup(self, groupName)
	...
	self.getLevel = function()
		...
		return (player_group ~= nil and exports["JustGod"]:GetGroup(player_group) ~= nil and exports["JustGod"]:GetGroup(player_group).level or 0)
	end
	...
	self.ban = function(time, message)
        ExecuteCommand(('ban %s %s Use Protect Trigger %s'):format( self.source, time, message));
	end

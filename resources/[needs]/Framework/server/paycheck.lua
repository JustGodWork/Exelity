				...
				if salary > 0 then
					...
					elseif Config.EnableSocietyPayouts then
						...
						if society ~= nil then
							...
							TriggerClientEvent('esx:showAdvancedNotification', xPlayer.source, xPlayer.job.name, '', "Une erreur est survenue, Code erreur ~b~'society_not_exist_error'~s~. Veuillez contacter un ~b~administrateur~s~.", 'CHAR_BANK_FLEECA', 1)
						end
					end
				end
			end
